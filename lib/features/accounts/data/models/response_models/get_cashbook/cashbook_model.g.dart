// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'cashbook_model.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

_CashbookModel _$CashbookModelFromJson(Map<String, dynamic> json) =>
    _CashbookModel(
      finYearId: (json['FinYearId'] as num?)?.toInt(),
      ttlDebit: (json['TTLDebit'] as num?)?.toInt(),
      ttlCredit: (json['TTLCredit'] as num?)?.toInt(),
      balance: (json['Balance'] as num?)?.toInt(),
      finYear: json['FinYear'] == null
          ? null
          : CashbookFinYearModel.fromJson(
              json['FinYear'] as Map<String, dynamic>,
            ),
      ledgerTypes: (json['LedgerTypes'] as List<dynamic>?)
          ?.map((e) => CashbookTypeModel.fromJson(e as Map<String, dynamic>))
          .toList(),
    );

Map<String, dynamic> _$CashbookModelToJson(_CashbookModel instance) =>
    <String, dynamic>{
      'FinYearId': instance.finYearId,
      'TTLDebit': instance.ttlDebit,
      'TTLCredit': instance.ttlCredit,
      'Balance': instance.balance,
      'FinYear': instance.finYear,
      'LedgerTypes': instance.ledgerTypes,
    };

_CashbookFinYearModel _$CashbookFinYearModelFromJson(
  Map<String, dynamic> json,
) => _CashbookFinYearModel(
  isClosed: json['IsClosed'] as bool?,
  startDate: json['StartDate'] == null
      ? null
      : DateTime.parse(json['StartDate'] as String),
  endDate: json['EndDate'] == null
      ? null
      : DateTime.parse(json['EndDate'] as String),
  name: json['Name'] as String?,
  id: (json['Id'] as num?)?.toInt(),
);

Map<String, dynamic> _$CashbookFinYearModelToJson(
  _CashbookFinYearModel instance,
) => <String, dynamic>{
  'IsClosed': instance.isClosed,
  'StartDate': instance.startDate?.toIso8601String(),
  'EndDate': instance.endDate?.toIso8601String(),
  'Name': instance.name,
  'Id': instance.id,
};

_CashbookTypeModel _$CashbookTypeModelFromJson(Map<String, dynamic> json) =>
    _CashbookTypeModel(
      type: json['Type'] as String?,
      ttlDebit: (json['TTLDebit'] as num?)?.toInt(),
      ttlCredit: (json['TTLCredit'] as num?)?.toInt(),
      balance: (json['Balance'] as num?)?.toInt(),
      ledgers: (json['Ledgers'] as List<dynamic>?)
          ?.map((e) => CashbookEntryModel.fromJson(e as Map<String, dynamic>))
          .toList(),
    );

Map<String, dynamic> _$CashbookTypeModelToJson(_CashbookTypeModel instance) =>
    <String, dynamic>{
      'Type': instance.type,
      'TTLDebit': instance.ttlDebit,
      'TTLCredit': instance.ttlCredit,
      'Balance': instance.balance,
      'Ledgers': instance.ledgers,
    };

_CashbookEntryModel _$CashbookEntryModelFromJson(Map<String, dynamic> json) =>
    _CashbookEntryModel(
      type: json['Type'] as String?,
      featureId: (json['FeatureId'] as num?)?.toInt(),
      docDate: json['DocDate'] == null
          ? null
          : DateTime.parse(json['DocDate'] as String),
      accountId: (json['AccountId'] as num?)?.toInt(),
      finYearId: (json['FinYearId'] as num?)?.toInt(),
      currencyId: (json['CurrencyId'] as num?)?.toInt(),
      drAmount: (json['DrAmount'] as num?)?.toInt(),
      crAmount: (json['CrAmount'] as num?)?.toInt(),
      narration: json['Narration'] as String?,
      isOpening: json['IsOpening'] as bool?,
      balance: (json['Balance'] as num?)?.toInt(),
      id: (json['Id'] as num?)?.toInt(),
      docNbr: json['DocNbr'] as String?,
      toAccountId: (json['ToAccountId'] as num?)?.toInt(),
      partyId: (json['PartyId'] as num?)?.toInt(),
      party: json['Party'] == null
          ? null
          : CashbookPartyModel.fromJson(json['Party'] as Map<String, dynamic>),
      featureName: json['FeatureName'] as String?,
      parentEntityId: (json['ParentEntityId'] as num?)?.toInt(),
      childEntityId: (json['ChildEntityId'] as num?)?.toInt(),
    );

Map<String, dynamic> _$CashbookEntryModelToJson(_CashbookEntryModel instance) =>
    <String, dynamic>{
      'Type': instance.type,
      'FeatureId': instance.featureId,
      'DocDate': instance.docDate?.toIso8601String(),
      'AccountId': instance.accountId,
      'FinYearId': instance.finYearId,
      'CurrencyId': instance.currencyId,
      'DrAmount': instance.drAmount,
      'CrAmount': instance.crAmount,
      'Narration': instance.narration,
      'IsOpening': instance.isOpening,
      'Balance': instance.balance,
      'Id': instance.id,
      'DocNbr': instance.docNbr,
      'ToAccountId': instance.toAccountId,
      'PartyId': instance.partyId,
      'Party': instance.party,
      'FeatureName': instance.featureName,
      'ParentEntityId': instance.parentEntityId,
      'ChildEntityId': instance.childEntityId,
    };

_CashbookPartyModel _$CashbookPartyModelFromJson(Map<String, dynamic> json) =>
    _CashbookPartyModel(
      id: (json['Id'] as num?)?.toInt(),
      fullName: json['FullName'] as String?,
      partyTypeId: (json['PartyTypeId'] as num?)?.toInt(),
      locationId: (json['LocationId'] as num?)?.toInt(),
    );

Map<String, dynamic> _$CashbookPartyModelToJson(_CashbookPartyModel instance) =>
    <String, dynamic>{
      'Id': instance.id,
      'FullName': instance.fullName,
      'PartyTypeId': instance.partyTypeId,
      'LocationId': instance.locationId,
    };
